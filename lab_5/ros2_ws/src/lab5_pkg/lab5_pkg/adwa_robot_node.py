...
class DWA_Node(Node):
    def __init__(self):
        ...
        self.collision_tolerance = 0.18   # m (should be 0.15 - 0.25)
        ...
        self.gamma = 1.5  # obstacle distance weight
        self.delta = 1.2  # target following weight

        self.optimal_target_dist = 0.5 # best following distance
        self.slowing_distance = 1.0 #distance threshold to slow down
    ...
    #     """
    #     AprilTag parameter
    #     Transform the range and bearing into X-Y coordinates expressed in the frame odom
    #     """
    ...
    def compute_dwa_cmd(self):
        ...
        ####task2 Precompute distance to target
        ...
        for v in v_values:
            for w in w_values:
                ...
                vel_score = self.score_velocity_adaptive(v,dist_to_goal_now) #go down when close
                target_score = self.score_target_following(traj) # keep dynamic distance
    ...
    def score_target_following(self, traj):
        """
        Task 2:follow dynamic target
        evaluate if it is close to optimal_target_dist。
        """
